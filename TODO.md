...
- Servono almeno due tabelle delle pagine diverse: una per il processo in esecuzione e una per il kernel. Visto che non vogliamo ricaricare il registro che contiene l'indirozzo della PGD ad ogni cambio di EL, abbiamo due registri `ttbr0_el1` e `ttbr1_el1`:
...
- Inizializziamo le tabelle per il kernel: definiremo una procedura assembly `__create_page_tables` che farà i seguenti passi
  ...
  - Visto che ogni tabella delle pagine è grande 1 pagina, dobbiamo allocare 3 pagine di memoria per mantenere queste 3 tabelle

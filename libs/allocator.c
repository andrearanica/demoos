...
void map_page(struct PCB* process, unsigned long virtual_address, unsigned long page_number) {
...
void map_table_entry(unsigned long* pte, unsigned long virtual_address) {
    unsigned long index = virtual_address >> PAGE_SHIFT;
    index = index & (PTRS_PER_TABLE - 1);
    unsigned long entry = virtual_address | MMU_PTE_FLAGS;
    pte[index] = entry;
}
